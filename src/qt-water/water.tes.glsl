...
uniform float noise_time_scale_factor = 1.0;
uniform float noise_scale_factor1 = 0.05;
uniform vec3  noise_scale_factor2 = vec3(0.075);
uniform vec3  noise_scale_factor3 = vec3(0.1);
...
void main(void)
{
  ...
  sim_time *= noise_time_scale_factor;
  vec3 uvw = wave + noise_scale_factor1 * vec3(snoise(wave + vec3(0.0, 0.0, sim_time)),
  ...
  wave += vec3(n) * noise_scale_factor2;
  normal += vec3(-n, 2.0 * n, -n) * noise_scale_factor3;
